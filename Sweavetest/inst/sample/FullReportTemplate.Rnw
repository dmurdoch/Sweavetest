...
\markright{"Your Class Name Here" Exam \Sexpr{with(.STEnv, versioncodes[testversion()])}}
...
\begin{center}
\fbox{\bf EXAM CODE \Sexpr{with(.STEnv, ExamNum[testversion()])}}
\end{center}
\vspace{0.5in}
...
\item { Fill in and {\bf code} your STUDENT NUMBER, SECTION and EXAM CODE on the Scantron sheet.  Your EXAM CODE is  \Sexpr{with(.STEnv, ExamNum[testversion()])}.  }
