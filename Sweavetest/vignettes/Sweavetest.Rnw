...
CorrectIndex.csv <- system.file("sample/CorrectIndex.csv", package = "Sweavetest")
@
...
<<StatisticalOverview,eval=FALSE,echo=TRUE>>=
...
latex(tabular((Heading(Section)*factor(Section)+1) 
          ~ Format(digits=2)*((Heading())*Percentage*Mean+(Heading())
            *Percentage*StdDev
...
\begin{table}
\caption{Output of the \texttt{StatisticalOverview} function. \label{tab:StatisticalOverview}}
\begin{center}
<<>>=
<<StatisticalOverview>>
@
...
<<StudentCount, echo=true, eval=false>>=
...
<<TestMeans, echo=true, eval=false>>=
...
<<ScoreDistribution,echo=true,eval=false>>=
weight <- max(seq_len(max(nchar(GradedTests$Correct))))
...
Using the \verb!CreateIndex()! function, a count of how many students
...
as ``TestIndex.dat"), as well as the result of the
\verb!grades()! function.
...
Below are the first five rows of the output matrix for our sample test:

<<echo=true,results=verbatim>>=
TestIndex <- read.csv(TestIndex.dat, head=TRUE)
NumberOfVersions <- 4
CreateIndex(TestIndex, GradedTests)[1:5,]
@
...
QuestionCounter <- 1
NumberOfVersions <- 4
.STEnv$Index <- read.csv(TestIndex.dat, head=TRUE)
AnswerCountMatrix <- CreateIndex(.STEnv$Index, GradedTests)
...
Options <- factor(paste(" ", c(1:length(AnswerCounts)), sep=""))
Resp <- 100*AnswerCounts/nrow(GradedTests)
Dis <- DistractorDiscrimination(GradedTests,QuestionCounter,.STEnv$Index)
...
CorrectIndex <- read.csv(CorrectIndex.csv)
...
Warnings(DR, ID, PB)
@
[No warnings are printed, as the example test had no obvious errors.]

A number of different outputs can be produced depending on which statistics are at unacceptable
levels. Some examples with modified data are given below:
