...
The output of this function is a matrix containing the number of
respondents choosing each answer option. The rows correspond to
question numbers, while the columns are representative of the answer
options. The answer options are in the same order that the instructor
provided them on the Sweave template.

Below are the first five rows of the output matrix for our sample test:

<<echo=true,results=verbatim>>=
TestIndex <- read.csv(TestIndex.csv, head=TRUE)
NumberOfVersions <- 4
...
QuestionCounter <- 1
NumberOfVersions <- 4
.STEnv$Index <- read.csv(TestIndex.csv, head=TRUE)
AnswerCounts <- CreateIndex(.STEnv$Index, GradedTests)
...
AnswerCounts <- subset(AnswerCounts, Question==1, 
                                    select=paste0("A", 1:5))
