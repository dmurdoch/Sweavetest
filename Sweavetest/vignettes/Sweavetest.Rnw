...
The report creation process uses the \pkg{tables} package \citep{pkg:tables} to format all the tables created by the \pkg{Sweavetest} package. This section will discuss the individual analysis tools used
...
TestIndex.dat <- system.file("sample/TestIndex.dat", package = "Sweavetest")
CorrectIndex.dat <- system.file("sample/CorrectIndex.dat", package = "Sweavetest")
@
...
When called, this package calls several functions that create the statistics which analyze the overall quality of the test. The function produces the following:
...
The table presents these statistics seperated by class section (if there is more than one section) and also for the class as a whole.
...
latex(tabular((Heading(Section)*factor(Section)+1) 
          ~ Format(digits=2)*((Heading())*Percentage*Mean+(Heading())*Percentage*StdDev
          +(Heading())*Percentage*Max+(Heading())*Percentage*Quantile75
          +(Heading())*Percentage*Median
          +(Heading())*Percentage*Quantile25
          +(Heading())*Percentage*Min), data=GradedTests))
\end{verbatim}
...
Using the \pkg{tables} package, the two tables are produced using the following syntax:
...
The Kuder-Richardson 20  is a well known statistic that measures the internal reliability of the test. It measures how consistently the items on the 
...
where K is the total number of items, $p_{i}$ is the probability of success on item i, $q_{i}$ is the probability of failure on item i, and $\sigma^{2}$ is the variance of the student scores.

\subsection{Ferguson's Delta}

The Ferguson's Delta statistic is produced by calling the \verb!FergusonsDelta()! function.
...
where n is the number of test takers, m is the number of items on the test, and $f_{s}$ is the number of test takers obtaining each possible test score.

Both Ferguson's Delta and the Kuder-Richardson 20 statistic are formatted into a table using the \pkg{tables} package.
...
answerCorrelations(Student,Correct)
\end{verbatim}
...
where C is the number of respondents answering correctly, and W is the number of respondents answering incorrectly.
...
A question with a difficulty rating less than .2 should be reviewed by the instructor.
...
\[ Item Discriminator = \frac{(Upper 25\% Correct) - (Lower 25\% Correct)}{Total Number of Respondents} \]
...
Similar to the output produced by the \verb!DifficultyRating()! function, one item discrimination value is produced per question.
...
Point biserial correlation can be easily calculated using the \verb!PointBiserial()! function.It takes as input the result of the \verb!grades()! function, and is called using the following syntax:

\begin{verbatim}
PointBiserial(GradedTests)
\end{verbatim}

This function creates one value of the point biserial index per question.
...
Using the \verb!CreateIndex()! function, a count of how many students chose each answer option is provided. This function takes as input the index automatically created by the test creation process (read in above as "TestIndex.dat"), as well as the result of the \verb!grades()! function.

The purpose od the \verb!CreateIndex()! function is to undo the answer choice randomization that is done during the test creation process, so that firm counts can be given for each answer choice, not just counts of how many students chose "A", "B", "C", etc.

The output of this function is a matrix containing the number of respondents choosing each answer option. The rows correspond to question numbers, while the columns are representative of the answer options. The answer options are in the same order that the instructor provided them on the Sweave template.

Below is the first five rows of the output matrix for our sample test:

\begin{verbatim}
CreateIndex{TestIndex.dat, GradedTests}
\end{verbatim}

\begin{verbatim}
<<>>=
TestIndex <- read.table(TestIndex.dat)
NumberOfVersions <- 4
AnswerCountMatrix <- CreateIndex(TestIndex, GradedTests)
AnswerCountMatrix[1:5,]
@
\end{verbatim}

Any answer option which has very few respondents should be reviewed by the instructor. If the answer choice is NA, this means that no such option was available on the test (i.e. there were only four answer choices, not five).
...
in the exact same way as described in the item discrimination section (where it is done for the question as a whole). Ideally, the only answer option with a positive item discrimination value will be the correct answer. All other options should have a negative item discrimination value, which tells the instructor that the students who scored
...
Both the option frequency and distractor discrimination statistics are combined in a single table for convenience. We provide the table produced for the first question of our sample test.
...
\begin{itemize}
\item The \verb!Version! variable to "Report"
\item The default "sample.dat" input for the \verb!readScanex()! function to the Scantron file they wish to use.
...
One important aspect of the Scantron file is the answer keys. They are denoted by Student ID 999999999, and have the same columns as the students. Thea answer keys for our sample test are provided below:
...
MasterClasslist <- system.file("sample/MasterClassListTemplate.csv", package = "Sweavetest")

SampleClassList <- read.csv(Classlist)
SampleMasterClassList <- read.csv(MasterClasslist)
@
...
Note that the column titles have been slightly modified by R. In the CSV file, the first column is entitled "Student #". All other periods in the title names correspond to blank spaces.
