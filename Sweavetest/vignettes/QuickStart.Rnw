...
appropriate, as it allows proofreading.  The \code{testversion()} call is not used at
this point, but we find it convenient to enter it now.
...
can produce 4 separately randomized versions of each test.)  With this
