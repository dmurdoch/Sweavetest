...
\pkg{Sweavetest} is not the only R package designed to produce tests
or exams.  The \pkg{exams} package \citep{Grun09,Zeileis12}  version
...
\citet{Macleod03} describes a system using Perl scripts to randomize
