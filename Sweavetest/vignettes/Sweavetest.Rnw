...
The \pkg{Sweavetest} package allows the instructor to export the
results of the \verb!grades()! function and merge it with a pre-existing
class list. This is done by calling the \verb!MergeClassList()! function.

This function takes as input the class list read in using the
\verb!read.csv! function, into which we are merging the results of
\verb!grades()!. A sample class list has been provided with this
package, and the class list used should mirror its format\footnote{Our class list
uses R function names in place of student names.  We expect few real classes
would have students named Dbinom Acf!}. A small
...
Note that the column titles have been slightly modified by \verb!read.csv!:
