...
\SweaveOpts{concordance=TRUE, keep.source=TRUE,echo=FALSE,results=tex,prefix.string=figs/}
<<>>=
options(width=70)
set.seed(18)
...
quality of the questions and of the test as a whole.  Sections \ref{sec:analysis} to \ref{sec:report} describe
this aspect of \pkg{Sweavetest}. Section \ref{sec:Merge} describes functions that help
with merging grade lists.  
It may be possible to use the aggregate results
...
\pkg{Sweavetest} is not the only software package designed to produce tests
or exams.  \citet{Macleod03} describes a system using Perl scripts to randomize
question order and answer order in automatically produced tests using
rich text format.

The R package \pkg{exams}  \citep{Grun09,Zeileis12}  version
1.9-6 is designed to produce exams in multiple formats (PDF, HTML,
etc.).  It also manages a database of questions in Sweave format,
allowing random selection of questions for each version of the test, and
randomization of the details of each question.  To achieve these aims,
it produces an R object containing the text of the question (using
\LaTeX\ markup), and then processes that object into various forms.
The \pkg{Sweavetest} package maintains this information in the main
\texttt{.Rnw} file, so it is more flexible in formatting, but less flexible in
output formats.  Currently \pkg{Sweavetest} has no support for random
selection of questions, but this is a feature that is being considered for
a future version.

The \pkg{ProfessR} package \citep{Lees13} also maintains a
database of questions, and produces an exam based on them.  It
produces a \LaTeX\ file directly from questions stored in an R object.
It does not appear to support randomized question content as 
\pkg{exams} and \pkg{Sweavetest} do via Sweave.

One difference between \pkg{Sweavetest} and the \citet{Macleod03} or
\citet{Grun09} approaches is that \pkg{Sweavetest} is designed to produce
a small number of randomized exams (typically 4), whereas those projects 
can easily produce a unique exam for each student.  Using a small
number of exams makes printing easier, since high speed duplication
of hundreds of identical papers is simpler than printing hundreds of
individually unique ones.  It also makes errors more obvious:  if every
exam is different, it is harder to detect an error that affects only a 
random subset than it is to detect a difference in a pre-identified 
fraction of the papers.  Finally, the \pkg{Sweavetest} limitation of always
asking the same questions is actually a benefit in the case where an
error slips past proofreading and needs to be corrected during the 
administration of the exam, since all students will have the same text
(with possible numerical variations) for corresponding question
numbers, and broadcast announcements of corrections are feasible.
...
The functions to randomize answers all work on the responses to
questions; the question text itself is not important, but it is assumed that
all questions have multiple choice responses.  See below for how to
handle tests where some questions are in other formats.

\pkg{Sweavetest} supports multiple (typically 4) separate versions of each test; questions
may contain from 2 to 5 answers.  The preamble to the test randomizes the test version
numbers to be codes from 100 to 999 in sorted order.  Version 1 of the test has the smallest 
...
enter the \verb!\item! macros if necessary.
...
Next, one calls the \texttt{Initialization()} and possibly the \texttt{newCommands()} function, for example
\begin{verbatim}
 <<>>=
 Initialization(name = "Test1", version="Teacher")
 newCommands(thesection = FALSE)
...
to ``Teacher'' during preparation of the test; this will mark correct answers with an asterisk,
and display an answer key at the end.  Use ``Student'' to produce a randomized version for the
...
convenient to use in the test; it may be called next.  Current definitions of simple macros
...
index automatically created by the test creation process, as well as the result of the
...
The full report can be used by those instructors who have created their entire test using \pkg{Sweavetest}. 
The creation of the test itself is discussed in Section \ref{sec:preparing}. 

By simply modifying the \texttt{Initialization()} call, the instructor can create a full report
to analyze the results of the test.  For example, change
...
set.seed(18)
library(Sweavetest)
Initialization("sample", "Report", scanex = "sample.dat")
\end{verbatim}
...
If the instructor did not use \pkg{Sweavetest} to create their test, analysis is still possible.

If the user only has the results of the test, they should use the low level template provided 
in \verb!system.file("sample/LowLevelTemplate.Rnw", package = "Sweavetest")!. 
This template produces many of the same statistics as the full report, but it will not 
display the text of the exam, nor will it be able to undo any randomization
in the answer options.

This method does however, provide the instructor with a function that will allow the report to recognize the 
correspondence between randomization in questions, as opposed to the randomization in answer options that
is done during the full report creation process. The function that must be called to determine these correspondences 
is the \verb!Unscramble()! function.
...
Order170 <- c(1:30)
Order738 <- c(15:30, 1:14)
Order840 <- c(20:30,1:10, 15:19,11:14)
Order967 <- c(30:1)
Orders <- data.frame("170"=Order170,"738"=Order738,
                             "840"=Order840,"967"=Order967, 
                             check.names=FALSE)
...
<<results=verbatim>>=
scanex[5:10, c("ExamCode", "Answers")]
@

First 6 student answers of new scanex

<<results=verbatim>>=
NewScanex[5:10, c("ExamCode", "Answers")]
@

We can see that the answers have been re-ordered in the new Scantron column. This will allow the 
\verb!LowLevelReport()! function to create a report with the proper
correspondences.
...
see Table \ref{tab:scantron} (page \pageref{tab:scantron}).

An example file is given below. The file should mirror the column titles of this file.

<<results=verbatim>>=
scanex[5:10,]
@

By convention, the answer keys are included in the Scantron file with
Student ID 999999999, and have the same columns as the students.
The answer keys for our sample test are provided below:

<<results=verbatim>>=
scanex[1:4,]
@

Note that the answer keys do not require a section number, and will not count in the total exam count.
...
The \pkg{Sweavetest} package allows the instructor to merge the
results of the \verb!grades()! function with a pre-existing
class list. This is done by calling the \verb!MergeClassList()! function.

This function takes as input the class list dataframe (typically read in using the
\verb!read.csv! function), into which we are merging the results of
\verb!grades()!. Column names should match those of the sample class list 
\verb!system.file("sample/ClassListTemplate.csv", package = "Sweavetest")!\footnote{Our class list
uses R function names in place of student names.  We expect few real classes
would have students named Dbinom Acf!}.
A small snapshot is provided below:
...
Note that the column titles have been slightly modified by \verb!read.csv! using
the \verb!make.names()! function in R:
blanks in title names have been converted to periods.
...
There are a couple optional variables under the instructors control. If the instructor has decided to exclude some of the questions from the exam (based on the analysis provided in the 
report), the \verb!weights! variable can be changed to a vector of 0s and 1s, corresponding to each question on the test. The user can change the name of the file to be exported from 
its default value of "MasterClassList", and the instructor can also request that one file be exported concatenating all class sections by changing the \verb!AcceptSectionCSV! variable to 
TRUE.
...
This function uses another function, the \verb!MergeLists()! function, to
actually append  the two files. The main purpose of the
\verb!MergeClassList()! function is to format the file so that it is clean
when it is exported back to the user.

Note that the \verb!MergeClassList()! function is not called
automatically at any point during the report creation process. This
function must be called separately by instructor if they wish to merge the
results into a pre-existing class list.
...
Being based on Sweave, \pkg{Sweavetest} processes the R code chunks in each exam as a script.  It maintains
an environment containing a number of global variables (accessed through ``getter'' and ``setter'' functions) that 
record changes to the state of the exam as the code chunks are processed.  These variables 
are described in the \texttt{?globals} help page.
